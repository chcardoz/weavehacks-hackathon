...
function FooterKeyCap({ children }: { children: React.ReactNode }) {
	return (
			<span className="inline-flex items-center justify-center w-5 h-5 rounded border border-white text-[10px] font-mono leading-none">
			{children}
		</span>
	);
}
	...
	return (
		<footer
			className={cn(
				...
				"bg-black",
				"text-sm font-medium text-neutral-300 transition-colors",
				className
			)}
		>
			{/* Left: Progress */}
			<div className="flex justify-start">
				{showProgress && (
					<span className="tabular-nums opacity-75 hover:opacity-100 transition-opacity">
							Slide {deck.index + 1} <span className="mx-1 text-neutral-500">/</span> {deck.total}
					</span>
				)}
			</div>
			...
			<div className="flex justify-center">
				{showAdd && (
					<span className="opacity-50 hover:opacity-100 transition-opacity flex items-center gap-1.5 text-xs">
						Built with <a href={"https://slide-cn.com"} target="_blank" rel="noreferrer" className="font-semibold hover:underline">Slide-CN</a>
					</span>
				)}
			</div>
