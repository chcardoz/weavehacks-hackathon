...
<Steps>
  ...
  <Step title="Escalate">
    You get an SMS plus a link to an AI voice note explaining the failure, and a deadline
    is armed in a Redis ZSET. Reply **1** (rollback), **2** (apply fix), or **3** (stop).
  </Step>
  ...
  <Step title="Recap">
    ...
    get an SMS recap with a link to the full Weave trace.
  </Step>
</Steps>
...
<CardGroup cols={3}>
  ...
  <Card title="The relay" icon="tower-broadcast">
    ...
    API-key issuance, Twilio SMS send + inbound reply webhook, and hosting the voice-note
    audio page. It never touches GPUs or code.
  </Card>
</CardGroup>
...
flowchart TD
    ...
    N --> O[Tag run, open PR, store memory, SMS recap + Weave link]
```

Every node above is a `@weave.op`, so one incident renders as one trace tree you can
share. The Weave link is included in your recap SMS.
